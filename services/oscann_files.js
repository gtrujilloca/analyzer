...
const CONTAINER_NAME_ENTRADABACKUP =
  process.env.CONTAINER_NAME_ENTRADABACKUP || 'entradabackup';
                ...
                getListFile(path, async (err, filesList) => {
                  try {
                    ...
                    if (indexJson !== -1) {
                      ...
                      const datajson = await updateJson(
                        path + '/' + files[i],
                        1
                      );
                      const res = await pushFilesAzure(
                        fileJson,
                        JSON.parse(jsonData)
                      );
                      console.log('res json' + res);
                      console.log('temine Subir a azure');
                      const datajson2 = await updateJson(
                        path + '/' + files[i],
                        -1
                      );
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      files.forEach(async file => {
        ...
        const existBlob = await veryBlob(
          CONTAINER_NAME_ENTRADA,
          jsonPaciente.Hospital +
            '/patologia_' +
            jsonPaciente.Label +
            '/paciente_' +
            blobName
